...
//ERRCME error for cme
func ERRCME(text string) (err error) {
	errText := strings.TrimSpace(text)
	if errID, cerr := strconv.Atoi(errText); cerr != nil {
		err = errors.New(errText)
	} else if errStr, ok := errCMEs[errID]; ok {
		err = errors.New(errStr)
	} else {
		err = fmt.Errorf("unhandle cme error: %d", errID)
	}
	return err
}

var errCMSs = map[int]string{
	1:   "Unassigned number",
...
//ERRCMS error for cms
func ERRCMS(text string) (err error) {
	errText := strings.TrimSpace(text)
	if errID, cerr := strconv.Atoi(errText); cerr != nil {
		err = errors.New(errText)
	} else if errStr, ok := errCMSs[errID]; ok {
		err = errors.New(errStr)
	} else {
		err = fmt.Errorf("unhandle cms error: %d", errID)
	}
	return err
}
